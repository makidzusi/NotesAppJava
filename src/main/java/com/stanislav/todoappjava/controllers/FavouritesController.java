...
public class FavouritesController {
    ...
    private void refreshNotes() {
        this.notes = this.notesService.getUserNotes(SessionManager.getCurrentUser().getId()).stream().filter(NoteModel::isFavourite).filter(note -> !note.isArchived()).toList();
    ...
    private void updateNotes(List<NoteModel> notes) {
        ...
        for (NoteModel note : notes) {
            try {
                ...
                controller.setOnFavouriteChanged(this::refreshNotes);
        ...
        if (notes.isEmpty()) {
            var emptyText = new Text("Пока нет заметок...");
